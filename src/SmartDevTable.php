        ...
        if (count($this->proses) > 0 && $this->showChk){
            if(count($arrSubHome)>0){
                ...
                foreach ($arrSubHome as $a => $b){
                    $out .= '<a href="javascript:void(0);" onclick="submenuHome($(this)); return false;" id = "'.rand().'new" act="'.$b[1].'" title="'.$a.'" '.($b[3]=="append" ? "data-append=\"true\" data-div=\"#newdiv\" data-class = \".formsearch"   : "").' '.($b[3]=="modal" ? "data-modal=\"true" : "").' ><i class="'. $b[4] .'"></i></a></span>';
